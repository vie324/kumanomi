...
supabase/
  ...
  migrations/
    ...
    0008_bootstrap_admin.sql  最初の管理者を作る(鶏と卵の解消)
  seed/
...
### 7-0. まず最初の管理者を作る(いちばん最初に必要)

名簿を登録できるのは「統括マネージャー以上または本部人事」ですが、
**いちばん最初はその人自身が名簿に居ません。**
そのままログインすると次のように出ます。

> ログインはできましたが、このアカウントがまだ社員名簿と紐付いていません。

これを解くために、SQL Editor から1回だけ実行する関数を用意しています。

```
1. Supabase → Authentication → Users → Add user
   メールアドレスとパスワードを決めてユーザーを作る
   (Auto Confirm User に必ずチェック。でないとログインできません)

2. SQL Editor で 1 行実行する
```
...
返ってくる JSON の `action` が `created`(新規)か `linked`(既存の名簿行に紐付け)です。
このメールとパスワードでアプリにログインできるようになります。

- **何度実行しても増えません。** 同じ人なら紐付け直すだけです
- 名簿を取り込み済みで**同じ氏名の行があれば、その行に紐付きます**(二重登録しません)
- **ブラウザからは呼べません。** SQL Editor か service_role からのみ実行できます
  (呼べてしまうと、誰でも自分を管理者にできてしまうため)
- すでに別の管理者が居るときは断ります。2人目以降は下の 7-1 の通常フローを使ってください

第3引数で社員番号、第4引数で権限(`ceo` / `exec` / `hr`)も指定できます。

```sql
select public.bootstrap_admin('hr@example.co.jp', '人事 花子', 'HR001', 'hr');
```

### ログインできないときに最初に見るところ

```sql
select * from public.v_login_status;
```

| status | 意味 | 対処 |
|---|---|---|
| `ログインできます` | 紐付け済み | — |
| `名簿に未登録` | Auth ユーザーだけある | `bootstrap_admin()` か名簿の取り込み |
| `社員番号が未設定` | 名簿にはあるが番号が空 | `fill_employee_numbers()` |
| `退職扱い` | `is_active = false` | 名簿を直す |
